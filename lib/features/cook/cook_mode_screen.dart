...
import '../../shared/ios_liquid_glass.dart';
...
final _cookDetailProvider = FutureProvider.autoDispose
    .family<SpiceRouteDetail, String>((ref, id) {
...
/// Recipe-image hero used when the recipe has an `imageUrl`. Shows the
/// full photo (letterboxed with [BoxFit.contain]) so long landscape
/// food shots aren't cropped to a thin strip, then dims the bottom with
/// a gradient so overlaid text stays legible.
class _HeroBand extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double cap = switch (dc) {
      DeviceClass.phone => 200,
      DeviceClass.tablet => 240,
      DeviceClass.desktop => 260,
      DeviceClass.wide => 280,
    };
    return SizedBox(
      height: cap,
      child: Stack(
        ...
        children: [
          ...
          CachedNetworkImage(
            imageUrl: imageUrl,
            httpHeaders: recipeImageHttpHeaders(imageUrl),
            fit: BoxFit.contain,
...
class _UnitsToggle extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final l = AppL10n.of(context);
    return AdaptiveLiquidGlassSegments<UnitSystem>(
      segments: const [
        UnitSystem.original,
        UnitSystem.metric,
        UnitSystem.imperial,
      ...
      labelFor: (u) => switch (u) {
        UnitSystem.original => l.cookUnitsOriginal,
        UnitSystem.metric => l.cookUnitsMetric,
        UnitSystem.imperial => l.cookUnitsImperial,
      },
    );
  }
}
...
class _NoStepsState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return CenterMessage(
      ...
      action: FilledButton(onPressed: onExit, child: Text(l.cookBackToRecipe)),
    );
  }
}
