...
import '../../state/my_recipes.dart' show invalidateMyRecipes;
...
final _detailProvider = FutureProvider.autoDispose
    .family<SpiceRouteDetail, String>((ref, id) async {
  ...
  Future<void> _confirmDelete(
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(l.detailDeletedToast)));
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return LayoutBuilder(
      builder: (context, constraints) {
        // Two-column layout when the modal is wide enough for image +
        // ingredients side-by-side. 720 was one px above the framed
        // width on a 768 dp iPad portrait (760 max − 56 margin = 704),
        // so every portrait tablet got a stacked layout despite having
        // plenty of horizontal room. 640 matches the explore filter row
        // wide/column gate and activates two-column on all tablets.
        ...
        return Row(
          ...
          children: [
            ...
            Expanded(
              flex: 5,
              child: _LeftColumn(recipe: recipe, scrollable: true),
            ),
...
class _LeftColumn extends ConsumerWidget {
  ...
  String _difficulty(AppL10n l, SpiceRouteDetail r) =>
      recipeDifficultyLabel(l, r.difficulty);
      ...
      child: Column(
        children: [
          ...
          Text(
            ...
            // Verbose locales (Vietnamese "T.GIAN CHUẨN BỊ", Korean
            // "준비 시간") wrap to multiple lines on the ~64-dp inner
            // card width we get at 320-dp viewport. Without these
            // guards, one of the three cards would silently grow
            // taller than its siblings, breaking the row's visual
            // rhythm. Two lines is the most we ever need.
            maxLines: 2,
