...
class CommunityPhoto {
  ...
  factory CommunityPhoto.fromDoc(
...
final communityPhotosProvider = StreamProvider<List<CommunityPhoto>>((
  ref,
) async* {
...
class CommunityUploadController extends StateNotifier<CommunityUploadState> {
  ...
  Future<void> upload({
    ...
    try {
      ...
      state = state.copy(uploading: false, error: _classifyFirestoreError(e));
    }
  }
...
/// Scroll the Explore page so the community live feed is visible.
void scrollCommunityLiveFeedIntoView(BuildContext context) {
  final target = CommunityBoard.liveFeedKey.currentContext;
...
class CommunityBoard extends ConsumerWidget {
  ...
  /// Anchor for [scrollCommunityLiveFeedIntoView] after a successful upload.
  static final liveFeedKey = GlobalKey();
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final feed = _LiveFeed(key: CommunityBoard.liveFeedKey);
...
class _ShowcaseCardState extends ConsumerState<_ShowcaseCard> {
  ...
  Future<void> _share() async {
    ...
    await ref
        .read(communityUploadProvider.notifier)
        .upload(
          photoBytes: bytes,
          cuisine: cuisine,
          author: _chef.text,
    ...
    if (st.lastSuccess) {
      ...
      // Bring the live feed on-screen so the user sees their photo land
      // in the grid — the board sits below the full recipe grid and is
      // easy to miss after upload.
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (!mounted) return;
        scrollCommunityLiveFeedIntoView(context);
      });
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(l.communitySharedToast),
          behavior: SnackBarBehavior.floating,
          action: SnackBarAction(
            label: l.communityViewFeed,
            onPressed: () => scrollCommunityLiveFeedIntoView(context),
          ),
        ),
      );
      ...
      Future<void>.delayed(const Duration(seconds: 4), () {
        if (mounted) {
          ref.read(communityUploadProvider.notifier).dismissSuccess();
        }
      });
    }
  }
...
class _LiveFeed extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final visible = cuisine == null
        ? all
        : all
              .where((p) => p.cuisine == null || p.cuisine == cuisine)
            ...
            if (post.cuisine != null)
              Positioned(
                ...
                child: Container(
                  ...
                  child: Text(
                    '${cuisineFlag(cuisineForDisplay(post.cuisine!))}  ${cuisineLabel(l, cuisineForDisplay(post.cuisine!))}',
                    style: theme.textTheme.labelSmall?.copyWith(
