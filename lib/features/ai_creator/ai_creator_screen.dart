...
class _LoadingPanelState extends State<_LoadingPanel> {
  Timer? _timer;
  // Monotonically increasing tick — we don't know the quote count from
  // inside `initState` (it depends on the localized list built in
  // `build`), so the modulo is applied at render time instead. This
  // way the carousel auto-adapts when a new ARB quote is added.
  int _tick = 0;
  ...
  Widget build(BuildContext context) {
    ...
    final index = _tick % quotes.length;
    return Container(
...
class _CreatorCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final createButton = FilledButton.icon(
      ...
      label: Text(
        state.hasRecipe ? l.aiCreatorRegenerate : l.aiCreatorCreateBtn,
      ),
    );
        ...
        children: [
          ...
          for (final ing in ingredients)
            Padding(
              ...
              child: Text(
                '• ${[if (ing['quantity'] != null) ing['quantity'].toString(), if (ing['unit'] != null) ing['unit'].toString(), if (ing['name'] != null) ing['name'].toString()].join(' ')}',
