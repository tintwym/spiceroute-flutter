...
/// Frosted-glass panel used by filter dropdowns.
///
/// On Flutter web, [BackdropFilter] repaints everything behind the menu
/// every frame — opening a course/dietary dropdown feels sluggish.
/// Web gets an opaque surface with the same tint; native keeps a lighter
/// blur (sigma 12, down from 24).
Widget buildFrostedSurface({
...
ThemeData buildTheme(Brightness brightness) {
  ...
  //
  // Dark mode wires the SAME semantic slots so widgets that pull from
  // `cs.outlineVariant`, `cs.surfaceContainer`, `cs.onSurfaceVariant`
  // etc. render with consistent meaning regardless of brightness.
  // Token values are tuned to the warm olive palette (NOT pure
  // neutrals) so the dark theme still reads as "the same product"
  // rather than a stock Material dark theme. Without this pass,
  // `fromSeed` would auto-derive sepia-tinted browns for dark
  // surfaces and the cream-on-charcoal reading experience would be
  // muddy.
  final colorScheme = isDark
      ? base.copyWith(
          primary: const Color(0xFF8FA46A),
          onPrimary: const Color(0xFF1B2010),
          secondary: const Color(0xFFE8B987),
          onSecondary: const Color(0xFF2A1F12),
          // Page surface — the darkest neutral. The whole app sits
          // on this in dark mode.
          surface: const Color(0xFF1B1F15),
          onSurface: const Color(0xFFEDE6D5),
          // Surface family from MORE elevated (lighter) to MOST
          // elevated. Material 3 convention in dark mode: elevation
          // = brightness. We hand-pick olive-tinted neutrals so the
          // ladder stays inside the brand palette instead of
          // collapsing to flat greys.
          surfaceContainerLowest: const Color(0xFF161A11),
          surfaceContainerLow: const Color(0xFF21261A),
          surfaceContainer: const Color(0xFF272C1F),
          surfaceContainerHigh: const Color(0xFF2F3526),
          surfaceContainerHighest: const Color(0xFF373E2C),
          // Bright surface — used by "lifted" widgets (active pills,
          // floating cards) to pop OFF the darker base. Brighter
          // than `surfaceContainerHighest` so the active state is
          // unambiguous against any neutral in the ladder.
          surfaceBright: const Color(0xFF424A35),
          // Secondary text + muted UI bits. Sits between the bright
          // onSurface and the page surface so subheads, hint text,
          // and disabled labels read as quieter without disappearing.
          onSurfaceVariant: const Color(0xFFB8B19E),
          // Dividers / hairlines around inputs + cards. One step
          // darker than `outline` so a clickable outline still wins
          // visual weight against a non-interactive divider.
          outline: const Color(0xFF6A6A55),
          outlineVariant: const Color(0xFF3A3D2E),
        )
      : base.copyWith(
