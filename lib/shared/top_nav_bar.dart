              ...
              child: ConstrainedBox(
                ...
                child: LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    // Flag-only pills are ~200 px wide (5 × ~36 + track
                    // padding). The old 540 px gate sat 4 px above the
                    // minimum tablet framed width (600 viewport − 64
                    // page pad = 536) which hid the ONLY language
                    // affordance in the top nav — [LanguageMenuButton]
                    // is phone-only and the account menu no longer
                    // carries a locale submenu.
                    final showFlags = w >= 480;
                    // Promote the labelled (endonym) variant once the
                    // viewport can afford it. The labelled row is
                    // ~430-470 px (5 pills × ~80-90 + container
                    // padding); on a typical 1280-1440 desktop the
                    // brand block + Spacer + this row + actions still
                    // fits with room to spare. Below 1100 px we fall
                    // back to flag-only so the row stays compact and
                    // doesn't push the trailing SIGN IN / avatar out
                    // of right-edge alignment with the body content.
                    ...
                    return SizedBox(
                      ...
                      child: Row(
                        children: [
                          ...
                          if (showFlags) ...[
                            LanguageFlagPills(showLabels: showLabels),
