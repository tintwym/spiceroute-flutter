...
class SavedRecipesController extends StateNotifier<SavedRecipesState> {
  SavedRecipesController(this._ref) : super(const SavedRecipesState()) {
    ...
    _ref.listen<Locale>(localeProvider, (prev, next) {
      if (prev?.languageCode != next.languageCode && state.ids.isNotEmpty) {
        unawaited(_hydrate(localeChanged: true));
      }
    });
  }
  ...
  Future<void> _hydrate({bool localeChanged = false}) async {
    ...
    if (localeChanged) {
      if (token != _hydrateToken) return;
      // Never keep the previous locale's titles on screen while the
      // batch re-fetch runs — mirrors Explore's localeChanged path.
      state = state.copyWith(
        loading: true,
        clearError: true,
        recipes: const [],
    ...
    state = state.copyWith(ids: pruned, recipes: mergedRecipes, loading: false);
  }
  ...
  Future<void> _mergeOnSignIn(AppUser user, {bool hydrateAfter = true}) async {
    ...
    try {
      ...
      final cloudIds =
          (snap.data()?['savedRecipeIds'] as List?)
              ?.whereType<String>()
              .toSet() ??
          const <String>{};
      ...
      if (merged.length == state.ids.length &&
          merged.length == cloudIds.length) {
        return;
      }
  ...
  void forgetDeleted(String id) {
    if (!state.ids.contains(id) && !state.recipes.any((r) => r.id == id)) {
      return;
    }
    final nextIds = state.ids.difference({id});
    final nextRecipes = state.recipes
        .where((r) => r.id != id)
  ...
  Future<void> _persistCloudDelta({
    ...
    try {
      final doc = <String, Object>{'updatedAt': FieldValue.serverTimestamp()};
      if (add.isNotEmpty) {
...
final savedRecipesProvider =
    StateNotifierProvider<SavedRecipesController, SavedRecipesState>((ref) {
