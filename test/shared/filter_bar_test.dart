...
void main() {
  group('trigger pill', () {
    testWidgets(
      ...
      (tester) async {
        ...
        // Hint text "All Requests" should render alone — no group
        // chip ahead of it.
    ...
    testWidgets(
      'course trigger shows category chip matching the screenshot',
      (tester) async {
  ...
  group('course accordion', () {
    ...
    testWidgets(
      'opens with the seven CourseGroup sections rendered',
      (tester) async {
        await _pumpFilterBar(tester);
    ...
    testWidgets(
      'group counts match the actual Course taxonomy',
      (tester) async {
        await _pumpFilterBar(tester);
        await openCourse(tester);
    ...
    testWidgets(
      'selecting a course from the accordion fires onChanged',
      (tester) async {
        Course? captured;
        await _pumpFilterBar(
          tester,
          onCourseChanged: (c) => captured = c,
        );
        await openCourse(tester);
    ...
    testWidgets(
      'opens with the selected course\'s group pre-expanded',
      (tester) async {
  ...
  group('accordion menu', () {
    ...
    testWidgets(
      'opens with all groups collapsed when no selection',
      (tester) async {
        await _pumpFilterBar(tester);
    ...
    testWidgets(
      'opens with the selected item\'s group pre-expanded',
      (tester) async {
        await _pumpFilterBar(tester, dietary: Dietary.quickEasy);
        // Tap the trigger to open the menu. We aim at the label
        // inside the trigger (Quick & Easy) rather than All Requests.
    ...
    testWidgets(
      'expand all opens every group and toggles its own label',
      (tester) async {
        await _pumpFilterBar(tester);
        await openDietary(tester);
    ...
    testWidgets(
      'search filters items and auto-expands groups with matches',
      (tester) async {
        await _pumpFilterBar(tester);
        await openDietary(tester);
    ...
    testWidgets(
      'search showing nothing renders the no-matches message',
      (tester) async {
        await _pumpFilterBar(tester);
        await openDietary(tester);
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        // "All Requests" should now be visible as a pinned reset row.
        expect(find.text('All Requests'), findsWidgets);

        await tester.tap(find.text('All Requests').last);
        await tester.pumpAndSettle();

        expect(captured, isNull,
            reason: 'tapping the reset row must report value=null '
                'so the parent clears its filter back to "all"');
      },
    );

    testWidgets(
      'search clear button wipes the query and restores all groups',
      (tester) async {
        ...
        expect(find.text('3 choices'), findsNWidgets(2),
            reason: 'Wellness + Cooking Formats both report their '
                'full 3-item count again after the search is cleared');
        expect(find.text('2 choices'), findsOneWidget,
            reason: 'Restrictions group reports its full count too');
      },
    );

    testWidgets(
      'group count reflects filtered visible items during search',
      (tester) async {
  ...
  group('mobile combined pill + tabbed sheet', () {
    ...
    Future<void> pumpMobile(
      ...
      // threshold and tall enough to give the tabbed sheet (which
      // clamps to a 280-dp minimum height) full breathing room
      // beneath the pill it anchors to.
      await _pumpFilterBar(
        tester,
        course: course,
        dietary: dietary,
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        expect(courseAgain.controller?.text, 'soup');
      },
    );
  ...
  group('trigger overflow', () {
    testWidgets(
      'narrow phone-width trigger never RenderFlex-overflows '
      'with a Burmese group label + long selection',
      (tester) async {
