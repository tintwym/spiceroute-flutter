  ...
  group('SavedRecipesController.forgetDeleted', () {
    test('removes the id and matching summary from state', () async {
      final container = ProviderContainer(
        overrides: [
          apiClientProvider.overrideWithValue(NoNetworkApi()),
        ],
      );
      ...
      expect(
        ctl.state.recipes.map((r) => r.id).toList(),
        equals(['a', 'c']),
      );
    });
    ...
    test('removes id even when recipes list is missing a matching summary',
        () async {
      ...
      ctl.state = const SavedRecipesState(
        ids: {'orphan-id'},
        recipes: [],
      );
